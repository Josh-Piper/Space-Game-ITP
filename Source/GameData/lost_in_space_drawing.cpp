...
void draw_next_level_information(const game_data &game) {
    ...
    if (timer_in_second > 0 && timer_in_second <= 3.0) {
        ...
        draw_text(LEVEL_UP_TEXT + to_string(game.game_level), TEXT_COLOR, FONT, TEXT_SIZE, LEVEL_UP_BOX_X + 5,
                  LEVEL_UP_BOX_Y + 5, option_to_screen());
        ...
        if (game.game_level == 2) {
            ...
            draw_text(" Space Fighters will not Spawn! Be Careful...", TEXT_COLOR, FONT, TEXT_SIZE - 10,
                      LEVEL_UP_BOX_X + 5, LEVEL_UP_BOX_Y + 56, option_to_screen());
        }

        if (game.game_level == 3) {
            fill_rectangle(COLOR_DARK_GRAY, LEVEL_UP_BOX_X, LEVEL_UP_BOX_Y + 55, 385, 30, option_to_screen());
            draw_text(" Level is Unsupported. Alpha Testing Mode ", TEXT_COLOR, FONT, TEXT_SIZE - 10,
                      LEVEL_UP_BOX_X + 5, LEVEL_UP_BOX_Y + 56, option_to_screen());
        }
    }
}
